...
import pivot from '../../src/operators/pivot';

let UNDEF;

describe('test pivot', () => {
  var salesTransactionDS = new DataStore(
  ...
  it('pivot takes 3 arguments', () => {
    ...
    result = pivotOps.fn(salesTransactionDT._data, salesTransactionDT._schema, salesTransactionDT._config);
  ...
  it('column in argument doesn\'t belong in dataview', () => {
    ...
    expect(() => pivotOps.fn(salesTransactionDT._data, salesTransactionDT._schema, salesTransactionDT._config)).toThrow();
  });
  ...
  describe('outputAs is used in group config', () => {
    it('single sorted column grouping', () => {
      let DS, DT,
        schemaArr = [{ name: 'Name', type: 'string' }, { name: 'Age' }, { name: 'Class', type: 'number' }],
        dataArr = [
          ['John', 13, 7],
          ['Luna', 17, 11],
          ['Sam', 5, 2],
      ...
      result = pivotOps.fn(DT._data, DT._schema, DT._config);
    ...
    it('multiple sorted column grouping', () => {
      ...
      expect(result.schema[0].name).toBe('Student Name');
      expect(result.schema[1].name).toBe('Student Age');
    });
    ...
    it('multiple unsorted column grouping', () => {
      ...
      DS = new DataStore(dataArr, schemaArr, { enableIndex: true, indexBy: 'Class' });
      DT = DS.getDataTable();
      pivotOps = pivot(
        [{
          column: 'Name',
  ...
  describe('outputAs is used in aggregation config', () => {
    it('single sorted column grouping, single column aggregation', () => {
      ...
      expect(result.schema[1].name).toBe('2 - Student Count');
    });
    ...
    it('multiple column grouping, single column aggregation', () => {
      ...
      expect(result.schema[2].name).toBe('2 - Student Count');
    });

    it('single column grouping, multiple column aggregation', () => {
      let DS, DT,
        schemaArr = [{ name: 'Name', type: 'string' }, { name: 'Age', type: 'number' }, { name: 'Class', type: 'number' }],
        dataArr = [
          ['John', 13, 7],
          ['Luna', 17, 11],
          ['Sam', 5, 2],
      ...
      expect(result.schema[1].name).toBe('2 - Student Count');
      expect(result.schema[4].name).toBe('2 - Student Average Age');
    });
    ...
    it('multiple column grouping, multiple column aggregation', () => {
      ...
      expect(result.schema[2].name).toBe('2 - Student Count');
      expect(result.schema[5].name).toBe('2 - Student Average Age');
    });
    ...
    it('missing outputAs uses [original column name - operation]', () => {
      ...
      expect(result.schema[1].name).toBe('2 - Age - avg');
    });
  });

  it('outputAs is used in result schema', () => {
    pivotOps = pivot(
      [{
        column: 'Time',
        timeUnit: DatetimeUnits.Hour
    ...
    expect(result.schema[1].name).toBe('Adjustment - Value - sum');
    ...
    expect(result.schema[1].name).toBe('Adjustment - Custom');
  });
  ...
  it('timeUnit used in date column', () => {
    ...
    expect(result.data[0][0].config.duration.name).toEqual(DatetimeUnits.Year.Unit);
  });
  ...
  it('timeUnit = month should work', () => {
    ...
    expect(result.data[0][0].start).toBe(+new Date(2016, 9, 1));
    expect(result.data[0][0].end).toBe(+new Date(2016, 10, 1));
  });

  it('timeUnit = quarter should work', () => {
    pivotOps = pivot(
      [{
        column: 'Time',
        timeUnit: DatetimeUnits.Quarter
      }],
      'Item',
      [{
        column: 'Value',
    ...
    expect(result.data[0][0].start).toBe(+new Date(2016, 9, 1));
    expect(result.data[0][0].end).toBe(+new Date(2017, 0, 1));
  });

  it('timeUnit = year should work', () => {
    pivotOps = pivot(
      [{
        column: 'Time',
        timeUnit: DatetimeUnits.Year
      }],
      'Item',
      [{
        column: 'Value',
    ...
    expect(result.data[0][0].start).toBe(+new Date(2016, 0, 1));
    expect(result.data[0][0].end).toBe(+new Date(2017, 0, 1));
  });
  ...
  it('outputFormat is used', () => {
    ...
    expect(result.data[0][0].toString()).toBe('30 October 2016, 9 AM');
  });
  ...
  it('default format if outputFormat is missing', () => {
    ...
    expect(result.data[0][0].toString()).toBe('Oct 30, 2016 09 hrs');
  });
  ...
  it('binSize makes interval a range', () => {
    ...
    expect(result.data[0][0].start).toBe(1477798200000);
    expect(result.data[0][0].end).toBe(1477809000000);
  });
  ...
  it('string binSize uses 1', () => {
    ...
    expect(result.data[0][0].start).toBe(1477798200000);
    expect(result.data[0][0].end).toBe(1477801800000);
  });
  ...
  it('startValue is used when provided', () => {
    ...
    expect(result.data[0][0].start).toBe(1477870200000);
    expect(result.data[0][0].end).toBe(1477873800000);
  });
  ...
  it('sum aggregation should work', () => {
    ...
    expect(result.data[0][12]).toBe(21.8);
    expect(result.data[1][12]).toBeCloseTo(145.98);
  });
  ...
  it('avg aggregation should work', () => {
    ...
    expect(result.data[0][1]).toBe(UNDEF);
    expect(result.data[0][12]).toBe(21.8);
    expect(result.data[1][12]).toBeCloseTo(12.16500);
  });
  ...
  it('count aggregation should work', () => {
    ...
    expect(result.data[0][1]).toBe(UNDEF);
    expect(result.data[0][12]).toBe(1);
    expect(result.data[1][12]).toBe(12);
  });
  ...
  it('min aggregation should work', () => {
    ...
    expect(result.data[0][1]).toBe(UNDEF);
    expect(result.data[0][12]).toBe(21.8);
    expect(result.data[1][12]).toBe(2.48);
  });
  ...
  it('max aggregation should work', () => {
    ...
    expect(result.data[0][1]).toBe(UNDEF);
    expect(result.data[0][12]).toBe(21.8);
    expect(result.data[1][12]).toBe(22.78);
  });
  ...
  it('first aggregation should work', () => {
    ...
    expect(result.data[0][1]).toBe(UNDEF);
    expect(result.data[0][12]).toBe(21.8);
    expect(result.data[1][12]).toBe(15.66);
  });
  ...
  it('last aggregation should work', () => {
    ...
    expect(result.data[0][1]).toBe(UNDEF);
    expect(result.data[0][12]).toBe(21.8);
    expect(result.data[1][12]).toBe(10.99);
  });
  ...
  it('stddev aggregation should work', () => {
    ...
    expect(result.data[0][1]).toBe(UNDEF);
    expect(result.data[0][12].stddev).toBe(0);
    expect(result.data[1][12].stddev).toBeCloseTo(6.98);
  });
  ...
  it('variance aggregation should work', () => {
    ...
    expect(result.data[0][1]).toBe(UNDEF);
    expect(result.data[0][12].variance).toBe(0);
    expect(result.data[1][12].variance).toBeCloseTo(48.736);
  });
...
describe('test pivot edge cases', () => {
  var DS, DT, pivotOps,
    result;

  it('empty data array returns empty result without error', () => {
    let dataArr = [], schemaArr = [{ name: 'Name', type: 'string' }, { name: 'Age', type: 'number' }, { name: 'Standard', type: 'number' }];
    ...
    expect(result.schema[0].name).toBe('Name');
  });
  ...
  it('first row empty returns result without error', () => {
    let dataArr = [
        ...
        ['John', 12, 6],
        ['Sam', 9, 4]
      ], schemaArr = [{ name: 'Name', type: 'string' }, { name: 'Age', type: 'number' }, { name: 'Standard', type: 'number' }];
    ...
    expect(result.data[0]).toEqual(['Sam', 7, 9, UNDEF, UNDEF, 12, UNDEF, UNDEF]);
  ...
  it('all aggregation column values as null returns result without error', () => {
    ...
    expect(result.schema.length).toBe(7);
  });
  ...
  it('all group column values as null returns result without error', () => {
    ...
    expect(result.data[0]).toEqual([null, 7, 9, 12, 13, 13, 15.5]);
  });
  ...
  it('all group column values as empty strings returns result without error', () => {
    ...
    expect(result.data.constructor).toBe(Array);
    expect(result.data.length).toBe(1);
    expect(result.data[0]).toEqual(['', 7, 9, 12, 13, 13, 15.5]);
  });
  ...
  it('group column values as empty strings returns result without error', () => {
    ...
    expect(result.data.constructor).toBe(Array);
    expect(result.data.length).toBe(4);
    expect(result.data[0]).toEqual(['', 5, UNDEF, UNDEF, 12, 15, UNDEF, UNDEF]);
  });

  it('all pivot column values as null returns result without error', () => {
    let dataArr = [
        ['Sam', 11, null],
        ['Harry', 14, null],
        ['John', 12, null],
    ...
    expect(result.data.constructor).toBe(Array);
    expect(result.data.length).toBe(3);
    expect(result.data[0][1]).toBe(9.75);
    expect(result.data[1][1]).toBe(13);
    expect(result.data[2][1]).toBeCloseTo(14.67);
    expect(result.schema.length).toBe(2);
  });

  it('all pivot column values as UNDEF/missing values returns result without error', () => {
    let dataArr = [
        ['Sam', 11],
        ['Harry', 14],
        ['John', 12],
  ...
  it('pivot column values with combination of UNDEF/missing and null values returns result without error', () => {
    let dataArr = [
        ['Sam', 11],
        ['Harry', 14],
        ['John', 12],
    ...
    expect(result.data.constructor).toBe(Array);
    expect(result.data.length).toBe(3);
    expect(result.data[0]).toEqual(['Sam', 7, 10.666666666666666]);
    expect(result.data[1][0]).toBe('John');
    expect(result.data[2][0]).toBe('Harry');
    expect(result.data[1][1]).toBe(UNDEF);
    expect(result.data[2][1]).toBe(UNDEF);
  ...
  it('0 can be min', () => {
    let dataArr = [
        ...
        ['A', 3, 2],
        ['C', 10, 2]
      ], schemaArr = [{ name: 'Item', type: 'string' }, { name: 'Value', type: 'number' }, { name: 'Class', type: 'number' }];
    ...
    DS = new DataStore(dataArr, schemaArr, { enableIndex: true, indexBy: 'Value' });
    DT = DS.getDataTable();
